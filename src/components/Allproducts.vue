      ...
      <div class="card__body">
        <div class="half">
          <div class="featured_text">
            ...
            <p class="price" style="color:white;">{{product.quantite}} {{product.unite}}</p>
          </div>
<!--           <div class="image">
        ...
        <div class="half">
          <div class="description">
            <p style="color:white;">{{product.designation}}</p>
          </div>
...
<!--         <div class="recommend">
        ...
        <p class="text-center"><input v-model="quantity" type="float" class="form-control" placeholder="Qty" min="1"/></p>
          <button type="button" @click="AddToCartbon(product)">Add to cart</button>
        </div>
      </div>
    </div>
  </main>
...
//     },
    ...
    methods: {
        AddToCartbon() {
            ...
            this.$store.commit('AddToCartbon', item)
        ...
        increaseQtt : function(){
            this.cartbon.add(this.item);
        },
        decreaseQtt : function(){
            this.cartbon.decrease(this.item.id);
        },
        enableEdition(){
    ...
    computed:{
        qtt:{
            get(){
                return this.cartbon.getQuantite(this.item.id);
            },
            set(value){
                if(value.length==0) return;
                this.cartbon.add(this.item, value);
            }
        }
    }
};
